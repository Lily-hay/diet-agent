(function () {
    ...
    function updateChatMessages() {
        ...
        if (messages) {
            messages.innerHTML = state.chat.messages.map(renderMessage).join("");
        ...
        if (form) {
            ...
            if (btn) {
                btn.textContent = state.chat.sending ? "发送中..." : "发送";
                btn.disabled = state.chat.sending;
            }
            const input = form.elements.message;
            if (input && !state.chat.sending) {
                input.focus();
            }
        }
    }
    ...
    function renderMealForm() {
        ...
        return `
            ...
            <form id="mealForm" class="form-grid">
                ...
                <p class="field-hint full">点击下方标签即可多选，再次点击取消选择；用餐时间为必选项。</p>
    ...
    function renderSlotPicker(key, label, selected) {
        ...
        return `
            <div class="field">
                <label>${escapeHtml(label)}${required ? "（必选）" : ""}</label>
                <div class="slot-chips" data-slot-group="${escapeHtml(key)}">
                    ${options.map((option) => {
                        const isSelected = selectedSet.has(option);
                        return `<button type="button" class="slot-chip${isSelected ? " selected" : ""}" data-action="toggle-slot" data-slot="${escapeHtml(key)}" data-value="${escapeHtml(option)}">${escapeHtml(option)}</button>`;
                    }).join("")}
                    ${selected.map((value) => `<input type="hidden" name="${escapeHtml(key)}" value="${escapeHtml(value)}">`).join("")}
                </div>
            </div>
        `;
    }
    ...
    function handleClick(event) {
        ...
        if (action === "set-source") {
            ...
            const group = target.closest(".slot-chips");
            if (!group) {
                return;
            }
            ...
            const currentValues = Array.from(group.querySelectorAll(".slot-chip.selected"))
                .map((chip) => chip.dataset.value);
            group.querySelectorAll(`input[type=hidden][name="${key}"]`).forEach((el) => el.remove());
